$(function () {
    $("#jqGrid").jqGrid({
        url: baseURL + 'business/banksuser/list',
        datatype: "json",
        colModel: [			
	...
	methods: {
		...
		saveOrUpdate: function (event) {
		    $('#btnSaveOrUpdate').button('loading').delay(1000).queue(function() {
                var url = vm.banksUser.id == null ? "business/banksuser/save" : "business/banksuser/update";
                $.ajax({
                    type: "POST",
                    url: baseURL + url,
                    contentType: "application/json",
                    data: JSON.stringify(vm.banksUser),
                    success: function(r){
                        if(r.code === 0){
		...
		del: function (event) {
            ...
            layer.confirm('确定要删除选中的记录？', {
               ...
               if(!lock) {
		            ...
		            $.ajax({
                        type: "POST",
                        url: baseURL + "business/banksuser/delete",
                        contentType: "application/json",
		...
		getInfo: function(id){
			$.get(baseURL + "business/banksuser/info/"+id, function(r){
                vm.banksUser = r.banksUser;
            });
		},
